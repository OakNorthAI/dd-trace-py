...
from ...ext import SpanTypes, gql, sql
...
config._add("dbapi2", dict(trace_fetch_methods=asbool(get_env("dbapi2", "trace_fetch_methods", default=False)),))
...
class TracedCursor(wrapt.ObjectProxy):
    ...
    def _trace_method(self, method, name, resource, extra_tags, *args, **kwargs):
        ...
        with pin.tracer.trace(name, service=service, resource=resource, span_type=SpanTypes.SQL) as s:
            ...
            # add the name of the GraphQL resolver as a tag on the span, if it
            # has been set.
            resolver_name = gql.RESOLVER_NAME.get()
            if resolver_name:
                s.set_tag("resolver.name", resolver_name)
